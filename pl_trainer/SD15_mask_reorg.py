...
class RegionalAttnSDTraining(SDTraining):
    def __init__(
            self, 
            *args, 
...
class RegionalAttnSDTrainingMaskAndBboxReorgAdaptiveText(RegionalAttnSDTraining):
...
class InstdiffSDTraining(RegionalAttnSDTrainingMaskAndBboxReorgAdaptiveText):
    def prepare_attn_kwargs_batch(
        self, 
        batch,
        device, 
        ...
        from regional_attention.region_reorganization import prepare_train_attention_kwargs_instdiff, prepare_empty_attention_kwargs_instdiff
        reorg_bboxes = batch['reorg_bboxes']
        reorg_labels = batch['reorg_labels']
        reorg_masks = batch['reorg_masks']
        reorg_union_masks = batch['reorg_union_masks']
        ...
        for l, m, um, b, iou in zip(reorg_labels, reorg_masks, reorg_union_masks, reorg_bboxes, reorg_ious):
            ...
            if rnd_number > 0.1:
                res.append(
                    prepare_train_attention_kwargs_instdiff(
                        reorg_bboxes=b,
                        reorg_labels=l,
                        reorg_masks=m,
                        reorg_union_masks=um,
                ...
                res.append(
                    prepare_empty_attention_kwargs_instdiff(
                        mask=m,
                        cross_attention_dim=self.unet.config.cross_attention_dim,
                        tokenizer=self.tokenizer,
                        text_encoder=self.text_encoder,
